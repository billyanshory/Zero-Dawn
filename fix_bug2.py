...
# I messed up and put ADMIN KUPON PANEL instead of ADMIN PIN PANEL in IDUL_ADHA_LACAK_HTML when I was doing the prior pass. Let's fix that.

# First, remove ADMIN KUPON PANEL from IDUL_ADHA_LACAK_HTML.
lacak_html_part = re.search(r"IDUL_ADHA_LACAK_HTML = '''(.*?)'''\s*IDUL_ADHA_PEMBAGIAN_CEK_HTML", content, re.DOTALL)
if lacak_html_part:
    lacak_html = lacak_html_part.group(1)
    # Remove the kupon panel entirely from lacak html
    new_lacak_html = re.sub(r"<!-- ADMIN KUPON PANEL -->.*?{% endif %}", "", lacak_html, flags=re.DOTALL)

    # Now add the correct ADMIN PIN PANEL
    admin_pin_panel_html = """
            <!-- ADMIN PIN PANEL -->
            {% if is_admin %}
    ...
    new_lacak_html = new_lacak_html.replace('<!-- Card Container -->', '<!-- Card Container -->\n' + admin_pin_panel_html)

    content = content[:lacak_html_part.start(1)] + new_lacak_html + content[lacak_html_part.end(1):]
