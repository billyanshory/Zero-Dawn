...
def get_imsakiyah_schedule():
    ...
    try:
        ...
        for m in months:
            ...
            api_resp = requests.get(url, timeout=15)
...
def api_calc_hijri():
    try:
        ...
        api_resp = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
        api_resp.raise_for_status()
...
def admin_qurban_peta():
    ...
    try:
        ...
        app.logger.error(f"Error loading peta: {e}", exc_info=True)
        from flask import flash
        flash(f"Terjadi kesalahan saat memuat peta: {str(e)}", "error")
        return redirect(url_for('admin_qurban_dashboard'))
...
def admin_qurban_distribusi_handover(slot_id):
    ...
    try:
        ...
        distributed_qty = request.form.get('distributed_qty')
        ...
        try:
            qty = int(distributed_qty) if distributed_qty else 0
            if qty < 1 or qty > (slot.total_quota or 0):
                flash('Jumlah diserahkan tidak valid atau melebihi kuota.', 'error')
                return redirect(url_for('admin_qurban_peta'))
        except (ValueError, TypeError):
            flash('Jumlah diserahkan tidak valid.', 'error')
            return redirect(url_for('admin_qurban_peta'))

        slot.handler_name = handler_name
        slot.distributed_count = qty
        slot.handover_time = datetime.datetime.now()
        slot.is_locked = True

        db.session.commit()
        flash(f'Berhasil merekam serah terima {qty} paket untuk {slot.rt_identifier}', 'success')

    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error handover: {e}", exc_info=True)
