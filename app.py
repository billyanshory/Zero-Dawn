...
<div class="min-h-screen bg-[#F5F0E8] font-sans pb-20">
    ...
    <div class="bg-[#1B4332] text-white py-8 px-5 md:px-8 shadow-xl relative overflow-hidden">
        <div class="max-w-4xl mx-auto relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-3xl font-bold mb-2 tracking-tight text-[#D4A017]">Absensi Panitia Qurban</h1>
                <p class="text-[#F5F0E8]/80 text-sm max-w-xl">
                    Silakan melakukan absensi kehadiran. Admin akan memverifikasi kehadiran Anda di lokasi.
                </p>
            </div>
            <div class="flex flex-col text-right">
                <span class="text-xs text-[#D4A017] font-semibold uppercase tracking-widest mb-1">Status Absen</span>
                <span class="text-lg font-mono font-bold">{{ 'Berjalan' if is_valid_window else 'Ditutup' }}</span>
                <span class="text-xs text-[#F5F0E8]/70">Batas: {{ cutoff_str }} WITA</span>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-5 md:px-8 mt-6 relative z-20 space-y-8">
        ...
        {% if is_admin %}
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-xl border-2 border-[#1B4332]/20 pt-20 md:pt-24">
            <h3 class="text-lg font-bold text-[#1B4332] mb-4 border-b border-gray-100 pb-3"><i class="fas fa-lock mr-2"></i>Panel Admin: Pengaturan Absen</h3>
            <form action="/admin/idul-adha/absen/settings" method="POST" class="flex flex-col md:flex-row gap-4 items-end">
                <input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
                <div class="flex-1">
                    <label class="block text-xs font-bold text-gray-600 mb-1">Jam Mulai (HH:MM)</label>
                    <input type="time" name="start_time" value="{{ start_time_str }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]">
                </div>
                <div class="flex-1">
                    <label class="block text-xs font-bold text-gray-600 mb-1">Jam Berakhir (HH:MM)</label>
                    <input type="time" name="cutoff_time" value="{{ cutoff_str }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]">
                </div>
                <button type="submit" class="bg-[#1B4332] text-white font-bold py-3 px-6 rounded-xl hover:bg-[#153426] transition shadow-lg w-full md:w-auto">Simpan Waktu</button>
            </form>
        </div>
        {% endif %}

        {% if not has_attended and is_valid_window and not is_admin %}
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-[#D4A017]/20 flex flex-col items-center text-center">
            <div class="w-20 h-20 bg-[#D4A017]/10 text-[#D4A017] rounded-full flex items-center justify-center mb-4">
                <i class="fas fa-fingerprint text-4xl"></i>
            </div>
            <h2 class="text-xl font-bold text-gray-800 mb-2">Absen Sekarang</h2>
            <p class="text-sm text-gray-500 mb-6">Pastikan Anda sudah berada di lokasi pemotongan hewan qurban.</p>
            <form action="/idul-adha/absen" method="POST">
                <input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
                <button type="submit" class="bg-[#1B4332] text-white font-bold py-4 px-10 rounded-full hover:bg-[#153426] transition shadow-xl transform hover:-translate-y-1 text-lg">
                    Hadir di Lokasi
                </button>
            </form>
        </div>
        {% elif not has_attended and not is_valid_window and not is_admin %}
        <div class="bg-red-50 rounded-3xl p-6 shadow-md border border-red-200 text-center">
             <i class="fas fa-times-circle text-4xl text-red-500 mb-3"></i>
             <h3 class="text-lg font-bold text-red-700">Waktu Absen Telah Habis</h3>
             <p class="text-red-600 text-sm">Anda tidak dapat melakukan absen karena sudah melewati batas waktu yang ditentukan.</p>
        </div>
        {% elif has_attended and not is_admin %}
        <div class="bg-green-50 rounded-3xl p-6 shadow-md border border-green-200 text-center">
             <i class="fas fa-check-circle text-4xl text-green-500 mb-3"></i>
             <h3 class="text-lg font-bold text-green-700">Anda Sudah Absen</h3>
             <p class="text-green-600 text-sm">Terima kasih. Silakan perhatikan layar untuk status verifikasi oleh Admin.</p>
        </div>
        {% endif %}

        <!-- List of Attendees -->
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-gray-100">
            <h3 class="text-lg font-bold text-[#1B4332] mb-4 border-b border-gray-100 pb-3">Daftar Kehadiran Panitia</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-gray-50 text-gray-600 font-bold">
                        <tr>
                            <th class="p-3 rounded-tl-lg">Nama Panitia</th>
                            <th class="p-3">Waktu Hadir</th>
                            <th class="p-3">Status Sistem</th>
                            <th class="p-3 rounded-tr-lg">Verifikasi Admin</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="p-3 font-mono text-xs text-gray-500">{{ a.check_in_time.strftime('%H:%M:%S') }}</td>
                            <td class="p-3">
                                {% if a.status == 'Hadir Pagi' %}
                                <span class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-bold">{{ a.status }}</span>
                                {% else %}
                                <span class="px-2 py-1 bg-red-100 text-red-700 rounded text-xs font-bold">{{ a.status }}</span>
                                {% endif %}
                            </td>
                            <td class="p-3">
                                {% if a.is_verified %}
                                <span class="text-green-600 font-bold text-xs"><i class="fas fa-check-double mr-1"></i>Diterima</span>
                                {% else %}
                                    {% if is_admin %}
                                    <form action="/admin/idul-adha/absen/verify/{{ a.id }}" method="POST" class="inline">
                                        <input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
                                        <button type="submit" class="bg-amber-100 hover:bg-amber-500 hover:text-white text-amber-700 font-bold py-1 px-3 rounded text-xs transition">Verifikasi</button>
                                    </form>
                                    {% else %}
                                    <span class="text-amber-500 font-bold text-xs"><i class="fas fa-hourglass-half mr-1"></i>Menunggu</span>
                                    {% endif %}
                                {% endif %}
                            </td>
                        </tr>
                        {% else %}
                        <tr><td colspan="4" class="p-4 text-center text-gray-500">Belum ada yang absen hari ini.</td></tr>
                        {% endfor %}
                    </tbody>
                </table>
            </div>
        ...
        <div class="w-full">
            ...
            <div class="w-full">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mb-8">
                    ...
                    {% if not is_valid_window and not is_admin %}
                    <!-- DISABLED LATE STATE -->
                    <a href="/idul-adha/absen" class="bg-red-50 p-5 md:p-8 rounded-3xl shadow-lg flex flex-col items-center justify-center h-36 md:h-48 border-2 border-red-500 relative overflow-hidden group hover:scale-105 transition-all duration-300">
                        ...
                        <span class="text-sm md:text-base font-bold text-red-700 group-hover:text-red-800">Terlambat</span>
                    ...
                    <a href="/idul-adha/absen" class="bg-white p-5 md:p-8 rounded-3xl shadow-lg shadow-amber-200/50 flex flex-col items-center justify-center card-hover h-36 md:h-48 border border-amber-50 group hover:scale-105 hover:shadow-2xl transition-all duration-300 relative cursor-pointer">
                        ...
                        <span class="text-[10px] text-amber-600 font-medium absolute bottom-3">Batas: {{ settings.absen_end_time or '08:30' }} WITA</span>
                    </a>
                    {% endif %}

                    <!-- FEATURE 1: Laporan Qurban -->
                    <a href="/idul-adha/laporan" class="bg-white p-5 md:p-8 rounded-3xl shadow-lg shadow-[#1B4332]/10 flex flex-col items-center justify-center card-hover h-36 md:h-48 border border-[#1B4332]/5 group hover:scale-105 hover:shadow-2xl transition-all duration-300">
                    ...
                    <a href="/qurban/lacak" class="bg-white p-5 md:p-8 rounded-3xl shadow-lg shadow-[#8B2635]/10 flex flex-col items-center justify-center card-hover h-36 md:h-48 border border-[#8B2635]/5 group hover:scale-105 hover:shadow-2xl transition-all duration-300">
                        <div class="bg-[#8B2635]/10 w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center mb-3 text-[#8B2635] group-hover:bg-[#8B2635] group-hover:text-white transition-colors">
                            <i class="fas fa-list-ol text-2xl md:text-3xl"></i>
                        </div>
                    ...
                    <a href="/idul-adha/panduan" class="bg-white p-5 md:p-8 rounded-3xl shadow-lg shadow-[#D4A017]/10 flex flex-col items-center justify-center card-hover h-36 md:h-48 border border-[#D4A017]/5 group hover:scale-105 hover:shadow-2xl transition-all duration-300">
                        ...
                        <span class="text-sm md:text-base font-semibold text-[#D4A017] group-hover:text-[#D4A017]">Panduan</span>
                    </a>
                </div>
            </div>
...
<div class="min-h-screen bg-[#F5F0E8] font-sans pb-20">
    ...
    <!-- Main Content -->
    <div class="max-w-4xl mx-auto px-5 md:px-8 mt-6 pt-20 md:pt-24 relative z-20 space-y-8">
...
<div class="min-h-screen bg-[#F5F0E8] font-sans pb-20 flex flex-col items-center justify-center p-4">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-xl border border-yellow-500/20 mb-8 w-full max-w-md mx-auto relative z-20">
                ...
                <form id="generate-kupon-form" class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="warga_nik" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]">
                    </div>
                    <div>
            ...
            <script>
                ...
                async function generateKupon() {
                    ...
                    if(!nik) { alert("Nama harus diisi"); return; }
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        <div class="p-6 md:p-8">
            ...
            <form action="/qurban/pembagian/cek" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-600 mb-1">Nama Lengkap Kepala Keluarga</label>
                    <input type="text" name="nama_kepala_keluarga" required placeholder="Contoh: Budi Santoso" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017] text-center">
                </div>
            ...
            <div class="overflow-y-auto max-h-[50vh] pr-2 custom-scrollbar space-y-4">
                ...
                <div class="border border-gray-200 rounded-2xl p-4 flex justify-between items-center gap-4 hover:border-[#1B4332]/30 transition-colors {{ 'bg-gray-50 opacity-60' if kupon.is_claimed else '' }}">
                    ...
                    <div>
                        ...
                        {% else %}
                        <form action="/admin/qurban/distribusi/tandai-selesai/{{ kupon.id }}" method="POST">
                            <input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
                            <button type="submit" class="bg-[#D4A017] text-[#1B4332] text-xs font-bold px-4 py-2 rounded-xl hover:bg-[#b58812] transition shadow-md border border-[#D4A017]/20 flex items-center gap-2">
                                <i class="fas fa-hand-holding-heart"></i> Serahkan
                            </button>
                        </form>
                        {% endif %}
                    </div>
                ...
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p>Belum ada kupon yang didaftarkan.</p>
                </div>
                {% endfor %}
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 text-gray-600">
                        <tr>
                            ...
                            <th class="p-4 font-bold uppercase tracking-wider text-[10px] text-right">Status Kunci</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        {% for slot in slots %}
                        <tr class="hover:bg-gray-50 transition {{ 'bg-[#8B2635]/5' if slot.is_locked else '' }}">
                            ...
                            <td class="p-4 text-right">
                                {% if slot.is_locked %}
                                <span class="text-[10px] bg-[#8B2635] text-white px-2 py-1 rounded font-bold uppercase"><i class="fas fa-lock"></i> Locked</span>
                                {% else %}
                                <span class="text-[10px] text-green-600 font-bold uppercase"><i class="fas fa-unlock"></i> Open</span>
                                {% endif %}
                            </td>
                        </tr>
                        {% else %}
            ...
            <div class="prose prose-sm md:prose-base max-w-none text-[#F5F0E8] bg-white text-gray-800">
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-[#F5F0E8] p-5 rounded-2xl mb-4 border border-[#D4A017]/30">
                        ...
                        <p class="text-2xl md:text-3xl text-right font-arabic leading-[2.5] mb-4 text-[#D4A017]" style="font-family: 'Amiri', serif;" dir="rtl">بِسْمِ اللهِ وَاللهُ أَكْبَرُ، اللَّهُمَّ هَذَا مِنْكَ وَلَكَ، هَذَا عَنْ [nama Sohibul Qurban]</p>
...
def ramadhan_tarawih_action():
    try:
        night = request.form.get('night_index', '')
        item = TarawihSchedule.query.filter_by(night_index=night).first()
        if item:
            item.imam = request.form.get('imam', '')
            item.penceramah = request.form.get('penceramah', '')
            item.judul = request.form.get('judul', '')
        else:
            item = TarawihSchedule(
                night_index=night,
                imam=request.form.get('imam', ''),
                penceramah=request.form.get('penceramah', ''),
...
def idul_adha_dashboard():
    ...
    start_str = AppSettings.query.get('absen_start_time').value if AppSettings.query.get('absen_start_time') else '06:30'
    end_str = AppSettings.query.get('absen_end_time').value if AppSettings.query.get('absen_end_time') else '08:30'
...
def idul_adha_absen():
        ...
        try:
            ...
            if not existing:
                ...
                if status == 'Hadir Pagi':
                    ...
                    flash('Absen tercatat sebagai Terlambat.', 'error')
            else:
                flash('Anda sudah absen hari ini.', 'warning')
        except Exception as e:
            db.session.rollback()
            app.logger.error(f"DB Error: {e}")
    ...
    # GET request
    today_start = current_time.replace(hour=0, minute=0, second=0)
    attendees = QurbanAttendance.query.filter(QurbanAttendance.check_in_time >= today_start).order_by(QurbanAttendance.check_in_time.desc()).all()
    has_attended = QurbanAttendance.query.filter(QurbanAttendance.name == username, QurbanAttendance.check_in_time >= today_start).first() is not None
...
@app.route('/admin/idul-adha/absen/settings', methods=['POST'])
def admin_absen_settings():
    if not session.get('is_admin'):
        return redirect(url_for('index'))
    ...
    try:
        if start_time:
            s_obj = AppSettings.query.get('absen_start_time')
            if not s_obj:
                s_obj = AppSettings(key='absen_start_time')
                db.session.add(s_obj)
            s_obj.value = start_time

        if cutoff_time:
            e_obj = AppSettings.query.get('absen_end_time')
            if not e_obj:
                e_obj = AppSettings(key='absen_end_time')
                db.session.add(e_obj)
            e_obj.value = cutoff_time

        db.session.commit()
        flash('Pengaturan jam absen berhasil disimpan.', 'success')
    except Exception as e:
        db.session.rollback()
...
@app.route('/admin/idul-adha/absen/verify/<int:id>', methods=['POST'])
def admin_absen_verify(id):
    if not session.get('is_admin'):
        return redirect(url_for('index'))
    try:
        att = QurbanAttendance.query.get(id)
        if att:
            att.is_verified = True
            db.session.commit()
            flash('Kehadiran berhasil diverifikasi.', 'success')
        else:
            flash('Data absen tidak ditemukan.', 'error')
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error verifying absen: {e}")
        flash('Terjadi kesalahan sistem.', 'error')
    return redirect(url_for('idul_adha_absen'))
...
@app.route('/api/qurban/stats/update', methods=['POST'])
@csrf.exempt
def api_qurban_stats_update():
    if not session.get('is_admin'):
    ...
    try:
        ...
        stats.total_cattle = data.get('total_cattle', 0)
        stats.total_goat = data.get('total_goat', 0)
        stats.total_meat_weight_kg = data.get('total_meat_weight_kg', 0.0)
        stats.total_packages_prepared = data.get('total_packages_prepared', 0)
        stats.total_packages_distributed = data.get('total_packages_distributed', 0)

        db.session.commit()
        return jsonify({'success': True})
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error updating Qurban stats: {e}", exc_info=True)
...
def qurban_pembagian_cek():
    if request.method == 'POST':
        try:
            ...
            nama_kk = data.get('nama_kepala_keluarga', '').strip() or data.get('nik', '').strip()
...
def api_qurban_generate_kupon():
    ...
    try:
        ...
        # check existing nik
        if DistribusiKupon.query.filter_by(nama_kepala_keluarga=nama_kk).first():
...
def admin_qurban_distribusi():
    ...
    rendered_content = render_template_string(IDUL_ADHA_PEMBAGIAN_ADMIN_HTML, slots=slots, kupons=kupons, get_slot_by_id=get_slot_by_id, is_admin=True, settings=get_settings())
    return render_template_string(BASE_LAYOUT, styles=STYLES_HTML, active_page='idul-adha', content=rendered_content, is_admin=True, settings=get_settings())
...
def admin_qurban_peta():
    ...
    try:
        ...
        total_rt = len(slots)
        total_quota = sum([s.total_quota or 0 for s in slots])
        total_distributed = sum([s.distributed_count or 0 for s in slots])
...
if __name__ == '__main__':
    with app.app_context():
        ...
        try:
            db.session.execute(text("ALTER TABLE qurban_attendance ADD COLUMN IF NOT EXISTS is_verified BOOLEAN DEFAULT FALSE"))
            db.session.execute(text("ALTER TABLE distribusi_kupon CHANGE nik nama_kepala_keluarga VARCHAR(255) NOT NULL"))
            db.session.commit()
        except Exception:
            db.session.rollback()
