...
<body class="text-gray-800 antialiased {{ 'ramadhan-mode' if hide_nav else '' }}">
    ...
    <header class="md:hidden fixed top-0 left-0 w-full z-50 {{ t_nav_bg }} shadow-sm px-4 py-3 flex justify-between items-center max-w-md mx-auto right-0">
        ...
        <div class="text-right">
            ...
            {% set t_border_badge = 'border-rose-200' if theme and theme.icon_text == 'text-rose-600' else 'border-emerald-200' %}
...
<input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
    ...
    <script>
        ...
        async function deleteJadwalMedis(id) {
            ...
            try {
                const res = await fetch('/orang-tua/api/jadwal/' + id, { method: 'DELETE', headers: {'X-CSRFToken': document.querySelector('meta[name="csrf-token"]').getAttribute('content')} });
                if(res.ok) {
                    loadJadwalTimelineMedis();
                }
