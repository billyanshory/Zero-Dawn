...
<div class="pt-20 md:pt-32 pb-32 px-5 md:px-8">
    ...
    <div id="modal-developer" class="fixed inset-0 z-[100] hidden">
        <div class="absolute inset-0 bg-white/95 backdrop-blur-xl animate-[slideUp_0.5s_ease-out] flex flex-col items-center justify-start text-center overflow-y-auto">
            ...
            <!-- Sticky Header -->
            <div class="sticky top-0 z-10 bg-white/95 backdrop-blur-xl w-full pt-6 pb-4 px-8 flex-shrink-0 flex flex-col items-center relative">
                <button onclick="closeModal('modal-developer'); stopDevAudio()" class="absolute right-8 top-6 bg-gray-100 w-10 h-10 rounded-full text-gray-600 hover:bg-gray-200 text-xl flex items-center justify-center">&times;</button>
            ...
            <div class="px-8 pb-16 flex flex-col items-center w-full" style="-webkit-overflow-scrolling: touch;">
                <div class="mb-8 mt-4">
...
def ramadhan_dashboard():
    if session.get('peran') not in ['guru', 'kepala_sekolah'] and not session.get('is_admin'):
        return redirect(url_for('index'))
    portfolios = StudentPortfolio.query.limit(100).order_by(StudentPortfolio.created_at.desc()).all()
...
def manifest():
    return jsonify({
        ...
        "short_name": "SLB App",
        "description": "Aplikasi Sekolah Luar Biasa Difabel",
        "start_url": "/",
        "id": "/",
        "scope": "/",
        "display": "standalone",
...
def arsip_portofolio():
    page = request.args.get('page', 1, type=int)
    pagination = StudentPortfolio.query.limit(100).order_by(StudentPortfolio.created_at.desc()).paginate(page=page, per_page=20, error_out=False)
