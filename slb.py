                ...
                function initBrankasDial() {
                    ...
                    function unlockVault() {
                        ...
                        fetch('/brankas_unlock', {
                            ...
                            body: JSON.stringify({ kode: document.getElementById('brankas_kode_input') ? document.getElementById('brankas_kode_input').value : '' })
...
<input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
                        <div>
                            ...
                            <select name="student_id" required class="w-full bg-rose-50/50 border border-rose-200 rounded-2xl p-4 text-gray-800 focus:ring-2 focus:ring-rose-400 focus:border-rose-400 outline-none transition-all placeholder-rose-300 font-medium appearance-none"><option value="">Pilih Siswa...</option>{% for s in siswa_list %}<option value="{{ s.id }}">{{ s.nama }}</option>{% endfor %}</select>
                        </div>
                        <div>
                ...
                while True:
                    ...
                    try:
                        ...
                        if current_owner and current_owner.decode() == worker_id:
                            ...
                            app.logger.warning("Scheduler lock lost — stopping renewal.")
                            break
                    except Exception:
