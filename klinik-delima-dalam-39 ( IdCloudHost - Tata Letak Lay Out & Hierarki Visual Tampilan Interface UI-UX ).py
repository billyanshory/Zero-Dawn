...
class Queue(db.Model):
    ...
    medical_action = db.Column(db.Text)
...
# --- RBAC CONFIGURATION ---
MENU_ITEMS = [
    {'route': '/antrean', 'label': 'Antrean', 'icon': 'group', 'roles': ['patient', 'admin', 'doctor']},
    {'route': '/symptom-checker', 'label': 'Cek Gejala', 'icon': 'local_hospital', 'roles': ['patient', 'doctor']},
    {'route': '/booking', 'label': 'Booking', 'icon': 'event_available', 'roles': ['patient', 'doctor']},
    {'route': '/profil-klinik', 'label': 'Profil Klinik', 'icon': 'local_hospital', 'roles': ['patient', 'admin', 'doctor']},
    {'route': '/booking-list', 'label': 'Daftar Janji', 'icon': 'event', 'roles': ['admin', 'doctor']},
    {'route': '/rekam-medis', 'label': 'Rekam Medis', 'icon': 'medical_information', 'roles': ['doctor']},
    {'route': '/stok-obat', 'label': 'Stok Obat', 'icon': 'medication', 'roles': ['admin', 'doctor']},
    {'route': '/surat-sakit', 'label': 'Surat Sakit', 'icon': 'medical_information', 'roles': ['doctor']},
    {'route': '/kasir', 'label': 'Kasir & Laporan', 'icon': 'point_of_sale', 'roles': ['admin', 'doctor']},
    {'route': '/database-pasien', 'label': 'Data Pasien', 'icon': 'storage', 'roles': ['admin', 'doctor']},
    {'route': '/pencarian-pasien', 'label': 'Cari Pasien', 'icon': 'search', 'roles': ['admin', 'doctor']},
    {'route': '/statistik', 'label': 'Statistik', 'icon': 'pie_chart', 'roles': ['doctor']},
    {'route': '/download-data', 'label': 'Unduh Data', 'icon': 'picture_as_pdf', 'roles': ['admin', 'doctor']},
    {'route': '/financial-dashboard', 'label': 'Keuangan', 'icon': 'show_chart', 'roles': ['doctor']},
    {'route': '/expiry-tracker', 'label': 'Expiry Alert', 'icon': 'warning', 'roles': ['admin', 'doctor']},
    {'route': '/receipt-list', 'label': 'Cetak Struk', 'icon': 'receipt', 'roles': ['admin', 'doctor']},
    {'route': '/wa-reminder', 'label': 'WA Reminder', 'icon': 'chat', 'roles': ['admin', 'doctor']},
    {'route': '/qr-pasien', 'label': 'Pasien QR', 'icon': 'qr_code', 'roles': ['admin', 'doctor']},
    {'route': '/peta-sebaran', 'label': 'Peta Penyakit', 'icon': 'map', 'roles': ['doctor']},
    {'route': '/prediksi-stok', 'label': 'Prediksi Stok', 'icon': 'show_chart', 'roles': ['doctor']},
    {'route': '/lab-results', 'label': 'Hasil Lab', 'icon': 'science', 'roles': ['admin', 'doctor']},
    {'route': '/audit-log', 'label': 'Audit Log', 'icon': 'history', 'roles': ['doctor']},
    {'route': '/backup-db', 'label': 'Backup DB', 'icon': 'security', 'roles': ['doctor']},
]
...
def database_pasien_page():
    ...
    if q:
        query = query.filter((Queue.name.like(f"%{q}%")) | (Queue.phone.like(f"%{q}%")))
        patients = [r.to_dict() for r in query.order_by(Queue.created_at.desc()).limit(50).all()]
    else:
...
def pencarian_pasien_page():
    ...
    patients = []
    if q:
        subquery = db.session.query(db.func.max(Queue.id).label('max_id')).filter(
            (Queue.name.like(f"%{q}%")) | (Queue.phone.like(f"%{q}%"))
        ).group_by(Queue.phone).subquery()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if setting:
                ...
                db.session.add(SiteSetting(key='clinic_open', value=new_val))
            db.session.commit()
...
def api_queue_status():
    ...
    if request.args.get('full'):
        ...
        waiting_list = [r.to_dict() for r in Queue.query.filter_by(status='waiting').order_by(Queue.created_at.asc()).all()]
        
        today = datetime.date.today().isoformat()
        history_list = [r.to_dict() for r in Queue.query.filter_by(status='done').filter(Queue.created_at.like(f"{today}%")).order_by(Queue.created_at.asc()).all()]
        cancelled_list = [r.to_dict() for r in Queue.query.filter_by(status='cancelled').filter(Queue.created_at.like(f"{today}%")).order_by(Queue.created_at.desc()).all()]
...
def api_patient_card():
    ...
    row = Queue.query.filter_by(phone=phone).order_by(Queue.created_at.desc()).first()
...
def api_patient_lab():
    ...
    patient_ids = [r.id for r in Queue.query.filter_by(phone=phone).all()]
    
    if not patient_ids:
        return jsonify([])
        
    results = [r.to_dict() for r in LabResult.query.filter(LabResult.patient_id.in_(patient_ids)).order_by(LabResult.created_at.desc()).all()]
...
@role_required(['admin', 'doctor'])
def api_stock_list():
    items = [r.to_dict() for r in MedicineStock.query.order_by(MedicineStock.name.asc()).all()]
    ...
    if request.method == 'POST':
        try:
            ...
            if setting:
                ...
                db.session.add(SiteSetting(key='dental_game_config', value=config_str))
            db.session.commit()
            return jsonify({'success': True})
        except Exception as e:
            db.session.rollback()
...
def financial_dashboard():
    ...
    ).filter_by(status='done').group_by('m').order_by(db.asc('m')).limit(12).all()
...
def print_receipt(id):
    ...
    time_source = p.get('finished_at') or p.get('created_at')
    date_str = time_source.split(' ')[0] if time_source else 'Unknown'
...
def lab_results_page():
    ...
    if q:
        ...
        patients = [r.to_dict() for r in Queue.query.order_by(Queue.created_at.desc()).limit(50).all()]
...
def upload_lab():
    ...
    if file and file.filename != '' and allowed_file(file.filename):
        ...
        try:
            db.session.add(LabResult(patient_id=patient_id, description=desc, file_path=filename))
            db.session.commit()
...
<style>
    ...
    .medical-logo-icon.material-icons { font-size: 2rem; color: {% if role in ['admin', 'doctor'] %}white{% else %}var(--green){% endif %}; }
    .medical-title { 
    ...
    .feature-btn .material-icons {
        font-size: 1.6rem;
        margin-bottom: 6px;
        color: var(--green);
    }
    .feature-btn span:not(.material-icons) {
        font-size: 0.7rem;
        font-weight: 700;
        text-align: center;
        line-height: 1.1;
    ...
    body.clean-mode .medical-logo-icon.material-icons,
    body.clean-mode .medical-title,
    body.clean-mode .feature-btn .material-icons,
    ...
    body.clean-mode .feature-btn:hover .material-icons,
    body.clean-mode .feature-btn.active .material-icons {
        color: #FF6D00 !important;
    }
    ...
    @media (max-width: 768px) {
        ...
        .medical-logo-icon.material-icons { font-size: 1.5rem; }
        .role-btn span:not(.material-icons) { display: none; } /* Hide text on small screens */
        .role-btn { padding: 8px; border-radius: 50%; width: 35px; height: 35px; justify-content: center; }
        .role-btn .material-icons { margin: 0; }
    }
...
<style>
    ...
    /* Dark Mode Banner Fixes */
    body.dark-mode .banner-grid-container {
        background: #121212;
        border-bottom-color: #333;
    }
    ...
    body.dark-mode .feature-banner-header {
    ...
    /* Clean Mode Banner Fixes */
    body.clean-mode .banner-grid-container {
        background: #000;
        border-bottom-color: #333;
    }
    body.clean-mode .feature-banner {
        border: 1px solid #333;
    }
    body.clean-mode .feature-banner-title {
        color: #fff;
    }
    body.clean-mode .feature-banner-header {
        border-bottom-color: #333;
    }
    body.clean-mode .banner-btn {
        background: #111;
        color: #fff;
        border-color: #333;
    }
    body.clean-mode .banner-btn:hover {
        border-color: #FF6D00;
        color: #FF6D00;
    }
</style>
        ...
        <form action="/login" method="POST">
            <div class="mb-3">
                <div class="input-group">
                    <span class="input-group-text bg-light"><span class="material-icons">person</span></span>
                    <input type="text" name="userid" id="loginUser" class="form-control" placeholder="Username" required>
                </div>
            </div>
            <div class="mb-4">
                 <div class="input-group">
                    <span class="input-group-text bg-light"><span class="material-icons">lock</span></span>
                    ...
                    <button class="btn btn-outline-secondary" type="button" onclick="toggleLoginPass()">
                        <span class="material-icons" id="login-eye-icon">visibility</span>
                    </button>
                </div>
            </div>
    ...
    <div class="login-modal-box" style="width: 95%; max-width: 450px; max-height: 85vh; overflow-y: auto;">
        ...
        <div id="card-login-step">
            ...
            <div class="input-group mb-3">
                <span class="input-group-text bg-white"><span class="material-icons text-success">chat</span></span>
        ...
        <div id="card-display-step" style="display:none;">
            <div class="hard-card-wrapper">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <span class="material-icons fs-3 rgba-white-5">local_hospital</span>
                </div>
            ...
            <div class="mt-3">
                <ul class="list-group list-group-flush small bg-transparent">
                    ...
                    <li class="list-group-item bg-transparent px-0">
                        <strong class="d-block text-warning mb-1"><span class="material-icons me-2">medical_information</span>Keluhan:</strong>
                        <div id="d-complaint" class="fw-bold text-dark">...</div>
                    </li>
                    <li class="list-group-item bg-transparent px-0">
                        <strong class="d-block text-info mb-1"><span class="material-icons me-2">medical_services</span>Diagnosa:</strong>
                        <div id="d-diag" class="fw-bold text-dark">...</div>
                    </li>
                    <li class="list-group-item bg-transparent px-0">
                        <strong class="d-block text-danger mb-1"><span class="material-icons me-2">medication</span>Resep Obat:</strong>
                        <div id="d-presc" class="fw-bold text-dark">...</div>
                    </li>
                     <li class="list-group-item bg-transparent px-0">
                        <strong class="d-block text-success mb-1"><span class="material-icons me-2">local_hospital</span>Tindakan:</strong>
...
<div id="myLabModal" class="login-modal-overlay">
    <div class="login-modal-box" style="width: 350px;">
        ...
        <div id="lab-login-step">
            <div class="input-group mb-3">
                <span class="input-group-text bg-white"><span class="material-icons text-primary">chat</span></span>
    ...
    <div class="login-modal-box" style="width: 350px; text-align: center; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(15px);">
        ...
        <div class="d-grid gap-3">
            <button onclick="document.getElementById('puzzleSelectModal').style.display='flex'; document.getElementById('gameMenuModal').style.display='none';" class="btn btn-outline-success btn-lg fw-bold py-3 rounded-pill shadow-sm" style="display:flex; align-items:center; justify-content:center; gap:10px;">
                <span class="material-icons fs-5">extension</span> Puzzle Gambar
            </button>
            <button onclick="openDevPopup()" class="btn btn-outline-info btn-lg fw-bold py-3 rounded-pill shadow-sm" style="display:flex; align-items:center; justify-content:center; gap:10px;">
                <span class="material-icons fs-5">dentistry</span> Perawatan Gigi
            </button>
        </div>
    </div>
</div>
    ...
    <div class="login-modal-box" style="width: 350px; text-align: center;">
        ...
        <div class="d-grid gap-3">
            <button onclick="openPuzzleGame()" class="btn btn-outline-dark fw-bold py-3 rounded-pill">
                <span class="material-icons me-2">image</span> Lukisan Monalisa
            </button>
            <button onclick="openMangkukGame()" class="btn btn-outline-warning fw-bold py-3 rounded-pill">
                <span class="material-icons me-2">wine_bar</span> Mangkuk Cina
            </button>
        </div>
    </div>
</div>
...
<div id="dentalModal" class="login-modal-overlay" style="display:none; z-index: 10001;">
    ...
    <div class="hard-card-glass-enhanced" id="dentalMainWrapper">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold mb-0 text-white" style="letter-spacing: 1px;"><span class="material-icons me-2">dentistry</span> PERAWATAN GIGI</h3>
            <div>
                <button class="btn btn-dark rounded-pill me-2 fw-bold px-3 shadow-sm" onclick="enableDevMode()" id="devToggleBtn">
                    <span class="material-icons me-2">settings</span> DEV MODE
                </button>
            ...
            <div class="game-area">
                ...
                <div class="d-flex justify-content-center gap-3 w-100">
                    <button class="tool-btn" id="btn-brush" onclick="toggleBrush()">
                        <span class="material-icons text-info">auto_fix_high</span> Sikat Gigi (Busa)
                    </button>
                    <button class="tool-btn" onclick="resetDentalGame()">
                        <span class="material-icons text-danger">redo</span> Reset
                    </button>
                </div>
            </div>
            ...
            <div id="devPanel">
                ...
                <div class="d-grid gap-2 mt-2">
                    <button class="btn btn-outline-warning fw-bold py-2" onclick="toggleCalibration()" id="calibBtn">
                        <span class="material-icons me-2">my_location</span> CALIBRATE CURSOR
                    </button>
                    <button class="btn btn-primary fw-bold py-3 shadow" onclick="saveSettings()">
                        <span class="material-icons me-2">save</span> SAVE CHANGES PERMANENTLY
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="hard-card-glass" id="gameContainer">
        ...
        <div class="game-controls">
            ...
            <div class="image-controls">
                ...
                <button onclick="document.getElementById('uploadGameImg').click()" class="btn-control"><span class="material-icons text-primary">camera_alt</span> Upload Foto</button>
                <button onclick="resetGameImage()" class="btn-control"><span class="material-icons text-danger">undo</span> Reset Default</button>
            </div>
        </div>
                ...
                data.forEach(item => {
                    cont.innerHTML += `
                        <div class="d-flex align-items-center justify-content-between p-2 border-bottom">
                            ...
                            <a href="/uploads/${item.file_path}" target="_blank" class="btn btn-sm btn-primary rounded-pill"><span class="material-icons">visibility</span></a>
                        </div>
                    `;
                });
            }
...
<body>
    ...
    <div class="container-fluid px-3 py-4">
        ...
        <div class="d-flex flex-column gap-3 mb-4">
            <button onclick="openLogin('admin')" class="btn text-start p-3 w-100 d-flex justify-content-between align-items-center" style="background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(10px); border-radius: 15px; border: 1px solid rgba(0,0,0,0.05); box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                <div class="d-flex flex-column">
                    <span class="fw-bold text-dark" style="font-size: 1.1rem;">Portal Admin Klinik</span>
                    <span class="small text-muted">Akses khusus staf admin</span>
                </div>
            ...
            <button onclick="openLogin('dokter')" class="btn text-start p-3 w-100 d-flex justify-content-between align-items-center" style="background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(10px); border-radius: 15px; border: 1px solid rgba(0,0,0,0.05); box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                <div class="d-flex flex-column">
                    <span class="fw-bold text-dark" style="font-size: 1.1rem;">Ruang Dokter & Medis</span>
                    <span class="small text-muted">Akses khusus tenaga kesehatan</span>
                </div>
        ...
        <!-- Layanan Pasien Grid -->
        <div class="mb-4">
            <h5 class="fw-bold mb-3 ms-2 text-dark">Layanan Pasien</h5>
            <div class="row g-3">
                <div class="col-6">
                    <a href="/antrean" class="btn text-center w-100 h-100 py-4 d-flex flex-column align-items-center" style="background: rgba(255, 255, 255, 0.9); border-radius: 20px; box-shadow: 0 8px 15px rgba(0,0,0,0.05); text-decoration: none;">
                        <span class="material-icons mb-2 text-success" style="font-size: 2.5rem;">group</span>
                        <span class="fw-bold text-dark" style="font-size: 0.9rem;">Ambil Antrean</span>
                    </a>
                </div>
                <div class="col-6">
                    <a href="/symptom-checker" class="btn text-center w-100 h-100 py-4 d-flex flex-column align-items-center" style="background: rgba(255, 255, 255, 0.9); border-radius: 20px; box-shadow: 0 8px 15px rgba(0,0,0,0.05); text-decoration: none;">
                        <span class="material-icons mb-2 text-primary" style="font-size: 2.5rem;">local_hospital</span>
                        <span class="fw-bold text-dark" style="font-size: 0.9rem;">Cek Gejala AI</span>
                    </a>
                </div>
                <div class="col-6">
                    <a href="/booking" class="btn text-center w-100 h-100 py-4 d-flex flex-column align-items-center" style="background: rgba(255, 255, 255, 0.9); border-radius: 20px; box-shadow: 0 8px 15px rgba(0,0,0,0.05); text-decoration: none;">
                        <span class="material-icons mb-2 text-warning" style="font-size: 2.5rem;">event_available</span>
                        <span class="fw-bold text-dark" style="font-size: 0.9rem;">Booking Janji</span>
                    </a>
                </div>
                <div class="col-6">
                    <a href="/profil-klinik" class="btn text-center w-100 h-100 py-4 d-flex flex-column align-items-center" style="background: rgba(255, 255, 255, 0.9); border-radius: 20px; box-shadow: 0 8px 15px rgba(0,0,0,0.05); text-decoration: none;">
                        <span class="material-icons mb-2 text-info" style="font-size: 2.5rem;">info</span>
                        <span class="fw-bold text-dark" style="font-size: 0.9rem;">Profil Klinik</span>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="mb-4">
            <h5 class="fw-bold mb-3 ms-2 text-dark">Informasi Tambahan</h5>
            <div class="accordion accordion-flush" id="infoAccordion" style="border-radius: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
                <div class="accordion-item border-0 border-bottom" style="background: rgba(255, 255, 255, 0.8);">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed bg-transparent fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne">
                            <span class="material-icons text-primary me-2">schedule</span> Jadwal Praktik Dokter
                        </button>
                    </h2>
                    <div id="flush-collapseOne" class="accordion-collapse collapse" data-bs-parent="#infoAccordion">
                ...
                <div class="accordion-item border-0 border-bottom" style="background: rgba(255, 255, 255, 0.8);">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed bg-transparent fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseTwo">
                            <span class="material-icons text-warning me-2">campaign</span> Pengumuman Kesehatan
                        </button>
                    </h2>
                    <div id="flush-collapseTwo" class="accordion-collapse collapse" data-bs-parent="#infoAccordion">
                        <div class="accordion-body small text-muted">
                            Tetap patuhi protokol kesehatan dasar. Gunakan masker apabila sedang mengalami gejala flu atau batuk.
                        </div>
                    </div>
                </div>
                <div class="accordion-item border-0" style="background: rgba(255, 255, 255, 0.8);">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed bg-transparent fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseThree">
                            <span class="material-icons text-success me-2">help_outline</span> Panduan Aplikasi
                        </button>
                    </h2>
                    <div id="flush-collapseThree" class="accordion-collapse collapse" data-bs-parent="#infoAccordion">
                        <div class="accordion-body small text-muted">
                            Untuk pendaftaran cepat, gunakan menu "Ambil Antrean" atau "Booking Janji". Hasil laboratorium dapat diakses melalui "Hasil Lab".
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mb-5 pb-5">
            <a href="tel:081241865310" class="btn btn-danger w-100 py-3 fw-bold shadow-lg d-flex justify-content-center align-items-center" style="border-radius: 15px; font-size: 1.1rem;">
                <span class="material-icons me-2">emergency</span> Hubungi Kontak Darurat
            </a>
        </div>
    </div>
...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <style>body{background:#f4f7f6; font-family: 'Plus Jakarta Sans', sans-serif;}</style>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
</head>
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-primary"><span class="material-icons me-2">biotech</span> Hasil Laboratorium & Rontgen</h2>
                            ...
                            <tr>
                                    ...
                                    {% if p.results %}
                                        ...
                                        {% for res in p.results %}
                                            <a href="/uploads/{{ res.file_path }}" target="_blank" class="btn btn-sm btn-outline-primary" title="{{ res.description }}">
                                                <span class="material-icons me-1">description</span> {{ res.description or 'Dokumen' }}
                                            </a>
                                        {% endfor %}
                                        </div>
                                    {% else %}
                                ...
                                <td>
                                    <button class="btn btn-sm btn-success" onclick="openUploadModal({{ p.id }}, '{{ p.name }}')">
                                        <span class="material-icons me-1">upload</span> Upload
                                    </button>
                                </td>
                            </tr>
                            {% endfor %}
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-primary"><span class="material-icons me-2">show_chart</span> Prediksi Stok Obat (Inventory Forecasting)</h2>
        
        <div class="card shadow border-0 rounded-4">
            <div class="card-body p-4">
                <div class="alert alert-info">
                    <span class="material-icons me-2">info</span> Sistem menggunakan algoritma <strong>Moving Average</strong> berdasarkan data pemakaian 7 hari terakhir.
                </div>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-primary"><span class="material-icons me-2">map</span> Peta Sebaran Penyakit (GIS Sederhana)</h2>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-primary"><span class="material-icons me-2">fact_check</span> Symptom Checker (Deteksi Dini)</h2>
        ...
        <div class="card shadow border-0 rounded-4" style="max-width: 700px; margin: auto;">
            <div class="card-body p-5">
                ...
                <button class="btn btn-primary w-100 btn-lg rounded-pill fw-bold" onclick="checkSymptom()"><span class="material-icons me-2">medical_services</span> ANALISIS GEJALA</button>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-dark"><span class="material-icons me-2">qr_code</span> Kartu Pasien Digital QR Code</h2>
            ...
            <div class="card-body p-5 text-center">
                <form method="get" class="mb-4">
                    <div class="input-group input-group-lg">
                        ...
                        <button class="btn btn-primary px-4"><span class="material-icons">search</span> CARI</button>
                    </div>
                </form>
                ...
                <div class="list-group text-start">
                    ...
                    <div class="list-group-item list-group-item-action d-flex justify-content-between align-items-center p-3" 
                        ...
                        <div>
                            ...
                            <small class="text-muted"><span class="material-icons me-1">phone</span> {{ p.phone }}</small>
                        </div>
                        <button class="btn btn-dark rounded-pill px-4"><span class="material-icons me-2">qr_code</span> GENERATE QR</button>
                    </div>
                    {% endfor %}
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title"><span class="material-icons me-2">qr_code</span> Kartu Pasien Digital</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
    ...
    <script>
        ...
        function printQR() {
            ...
            win.document.write(`<div style="text-align:center; padding:50px; font-family: 'Plus Jakarta Sans', sans-serif;"><h2>${name}</h2><img src="${img}" width="300"><br><br>KLINIK KESEHATAN</div>`);
            win.print();
        }
    </script>
    <footer class="text-center py-4 mt-auto" style="display: none; background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border-top: 1px solid rgba(0,0,0,0.1);">
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-dark"><span class="material-icons me-2">history</span> Audit Trail (Kotak Hitam Digital)</h2>
        <div class="card shadow border-0 rounded-4">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-12">
                 <div class="glass-panel-custom">
                    <h4 class="mb-4 fw-bold"><span class="material-icons me-2 text-success">confirmation_number</span> Ambil Nomor Antrean</h4>
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6 d-flex align-items-center justify-content-center">
                            ...
                            <div id="ticket-placeholder" class="text-center text-muted opacity-50">
                                <span class="material-icons mb-3">star</span>
...
<body>
    ...
    <div class="container-fluid py-4 px-lg-5">
        <div class="row g-4">
            ...
            <div class="col-lg-12">
                <div class="glass-panel-custom">
                    <div class="section-label"><span class="material-icons me-2">medical_services</span> Sedang Diperiksa</div>
            ...
            <div class="col-lg-5">
                <div class="glass-panel-custom h-100">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="section-label mb-0"><span class="material-icons me-2">pending_actions</span> Antrean Menunggu</div>
            ...
            <div class="col-lg-7">
                <div class="glass-panel-custom mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="section-label mb-0"><span class="material-icons me-2">history</span> Riwayat Pemeriksaan (Hari Ini)</div>
                        <button class="btn btn-sm btn-outline-success fw-bold" onclick="openArchive()"><span class="material-icons me-2">storage</span> Database</button>
                    </div>
                ...
                <div class="glass-panel-custom bg-light border-danger">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="section-label text-danger mb-0"><span class="material-icons me-2">delete</span> DATA PASIEN YANG DIHAPUS</div>
                        <button class="btn btn-sm btn-outline-danger fw-bold" onclick="openArchive()"><span class="material-icons me-2">storage</span> Database</button>
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="submitFinish()"><span class="material-icons me-2">save</span> Simpan & Selesai</button>
                </div>
            </div>
        </div>
    </div>
        ...
        function loadData() {
            fetch('/api/queue/status?full=1').then(r => r.json()).then(data => {
                    ...
                    list.innerHTML += `
                        <div class="queue-card d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <small class="text-muted"><span class="material-icons me-1">medical_information</span> ${escapeHtml(p.complaint)}</small>
                            </div>
                            <button class="btn btn-sm btn-outline-primary fw-bold" onclick="callPatient(${p.id})">
                                <span class="material-icons me-1">campaign</span> Panggil
                            </button>
                        </div>
                    `;
                });
                ...
                if (data.current) {
                        ...
                        <div class="active-patient-box">
                            <div class="row align-items-center">
                                <div class="col-md-8 text-md-start mb-3 mb-md-0">
                                    ...
                                    <p class="lead mb-0 text-muted"><span class="material-icons me-2">medical_information</span> Keluhan: ${escapeHtml(data.current.complaint)}</p>
                                </div>
                                <div class="col-md-4 text-md-end">
                                    <button class="btn btn-success btn-lg w-100 py-3 shadow" onclick="openFinishModal(${data.current.id})">
                                        <span class="material-icons me-2">check_circle</span> Selesai Periksa
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    panel.innerHTML = '<div class="text-center text-muted py-5 border rounded bg-light"><h5><span class="material-icons fs-3 mb-3 d-block">person_off</span>Belum ada pasien yang dipanggil</h5><p>Silakan panggil pasien dari daftar antrean.</p></div>';
                }
                ...
                <div class="col-12 h-100">
                    <div class="glass-panel-custom h-100 m-0" style="border-radius:0; overflow-y:auto; border:none;">
                        <div class="d-flex justify-content-between align-items-center mb-4 sticky-top bg-white p-3 shadow-sm">
                            <h3 class="fw-bold text-success m-0"><span class="material-icons me-2">storage</span> Arsip Data Pasien</h3>
                            <div class="d-flex gap-2">
                                ...
                                <button class="btn btn-danger rounded-circle shadow-sm" onclick="closeArchive()"><span class="material-icons">close</span></button>
                            </div>
                        </div>
                        <div id="archive-content" class="container py-3">
                            <div class="text-center text-muted"><span class="material-icons fs-3">star</span><br>Loading Database...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function renderArchive() {
        ...
        dates.forEach(date => {
            ...
            let html = `<div class="date-header" onclick="toggleDate('${date}')">
                            <span><span class="material-icons me-2">event</span> ${dateFormatted}</span>
...
<body>
    ...
    <div class="container py-5">
        <div class="row g-4">
            <div class="col-md-12">
                <div class="glass-panel-custom">
                    <div class="section-label"><span class="material-icons me-2">add_circle</span> Tambah / Update Stok Obat</div>
                    <form id="add-stock-form" class="row g-3 align-items-end" onsubmit="addStock(event)">
                        ...
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-primary w-100 fw-bold"><span class="material-icons me-1">save</span> Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="glass-panel-custom">
                    <div class="section-label"><span class="material-icons me-2">list_alt</span> Daftar Stok Obat</div>
                    <div class="table-responsive">
                        <table class="table table-hover align-middle">
                            <thead class="table-light">
                                <tr>
        ...
        <td class="text-end" style="white-space: nowrap;">
            <div class="d-flex justify-content-end align-items-center gap-2">
                <div class="btn-group" role="group">
                    <button class="btn btn-sm btn-outline-danger" onclick="updateStock(${item.id}, -1)"><span class="material-icons">remove</span></button>
                    <button class="btn btn-sm btn-outline-success" onclick="updateStock(${item.id}, 1)"><span class="material-icons">add</span></button>
                </div>
                <button class="btn btn-sm btn-light text-danger" onclick="deleteStock(${item.id})" title="Hapus"><span class="material-icons">delete</span></button>
                                </div>
                            </td>
                        </tr>
                    `;
...
<div class="top-bar">
    ...
    <div class="top-bar-right d-flex align-items-center gap-3">
        ...
        <div class="d-none d-lg-flex gap-3 align-items-center">
            <a href="https://wa.me/6281241865310" class="social-icon-link" target="_blank"><span class="material-icons">chat</span></a>
            <a href="https://maps.app.goo.gl/4deg1ha8WaxWKdPC9" class="social-icon-link" target="_blank"><span class="material-icons">location_on</span></a>
            <a href="https://www.instagram.com/rivkycahyahakikiori/" class="social-icon-link" target="_blank"><span class="material-icons">camera_alt</span></a>
        </div>
    </div>
</div>
...
<div class="main-navbar">
    ...
    <button class="d-lg-none btn border-0" onclick="toggleMobileMenu()"><span class="material-icons fs-3">menu</span></button>
...
<style>
    ...
    .countdown-timer {
        ...
        text-align: center; margin: 20px 0; font-family: 'Plus Jakarta Sans', sans-serif;
    }
    .agenda-card-barca {
            ...
            <div class="d-flex flex-wrap justify-content-center align-items-center gap-4">
                ...
                <div class="position-relative d-flex align-items-center flex-column">
                    ...
                    <div class="position-absolute top-0 start-50 translate-middle-x d-flex gap-1" style="z-index: 10;">
                        <button class="trash-btn" onclick="deleteItem('sponsors', '{{ sponsor.id }}')"><span class="material-icons">delete</span></button>
                    </div>
                    <form action="/upload/sponsor/{{ sponsor.id }}" method="post" enctype="multipart/form-data" class="position-absolute bottom-0 end-0">
                        ...
                        <label for="sp-upload-{{ sponsor.id }}" class="btn btn-sm btn-light border rounded-circle p-1" style="width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer;"><span class="material-icons text-primary">camera_alt</span></label>
                    </form>
                    <div class="mt-2">
        ...
        <div class="container position-relative">
            ...
            <div class="horizontal-scroll-container">
                ...
                <div class="person-card" onclick="openPersonModal('{{ player.id }}', '{{ player.name }}', '{{ player.position }}', '{{ '/uploads/' + player.image_path if player.image_path else '' }}', '{{ player.nationality }}', '{{ player.joined }}', '{{ player.matches }}', '{{ player.goals }}')">
                    ...
                    {% else %}
                        <div class="person-img d-flex align-items-center justify-content-center bg-secondary"><span class="material-icons fs-1 text-white-50">person</span></div>
                    {% endif %}
                    ...
                    <div class="position-absolute top-0 end-0 p-2 d-flex gap-2" onclick="event.stopPropagation()">
                        <button class="btn btn-sm btn-danger" onclick="deleteItem('personnel', '{{ player.id }}')"><span class="material-icons">delete</span></button>
                    </div>
                    {% endif %}
                </div>
                {% endfor %}
                ...
                <div class="person-card d-flex align-items-center justify-content-center" onclick="addCard('personnel', 'player')">
                    <span class="material-icons fs-1 text-success">add</span>
                </div>
                {% endif %}
            </div>
        </div>
            ...
            <div class="horizontal-scroll-container">
                ...
                <div class="person-card" onclick="openPersonModal('{{ coach.id }}', '{{ coach.name }}', '{{ coach.position }}', '{{ '/uploads/' + coach.image_path if coach.image_path else '' }}', '{{ coach.nationality }}', '{{ coach.joined }}', '{{ coach.matches }}', '{{ coach.goals }}')">
                    ...
                    <div class="position-absolute top-0 end-0 p-2 d-flex gap-2" onclick="event.stopPropagation()" style="z-index: 10;">
                        <button class="btn btn-sm btn-danger" onclick="deleteItem('personnel', '{{ coach.id }}')"><span class="material-icons">delete</span></button>
                    </div>
                    {% endif %}
                </div>
                {% endfor %}
            ...
            <div class="horizontal-scroll-container">
                ...
                <div class="person-card" onclick="openPersonModal('{{ mvp.id }}', '{{ mvp.name }}', '{{ mvp.position }}', '{{ '/uploads/' + mvp.image_path if mvp.image_path else '' }}', '{{ mvp.nationality }}', '{{ mvp.joined }}', '{{ mvp.matches }}', '{{ mvp.goals }}')">
                    ...
                    <div class="person-info">
                     ...
                     {% if admin %}
                    <div class="position-absolute top-0 end-0 p-2 d-flex gap-2" onclick="event.stopPropagation()">
                        <button class="btn btn-sm btn-danger" onclick="deleteItem('personnel', '{{ mvp.id }}')"><span class="material-icons">delete</span></button>
                    </div>
                    {% endif %}
                </div>
                {% endfor %}
                ...
                <div class="person-card d-flex align-items-center justify-content-center" onclick="addCard('personnel', 'mvp')">
                    <span class="material-icons fs-1 text-warning">add</span>
                </div>
                {% endif %}
            </div>
        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="agenda-card-barca cursor-pointer" onclick="openAgendaModal('{{ item.id }}', '{{ item.title }}', '{{ item.event_date }}', '{{ item.price }}', '{{ item.image_path if item.image_path else '' }}')">
                                ...
                                {% if admin %}
                                <button class="position-absolute top-0 end-0 btn btn-sm btn-danger m-1" style="z-index:5;" onclick="deleteItem('agenda_content', '{{ item.id }}'); event.stopPropagation();"><span class="material-icons">delete</span></button>
                                {% endif %}
                            </div>
                        </div>
                        {% endfor %}
            ...
            <div class="row">
                ...
                <div class="col-md-5 order-md-2 mb-3 mb-md-0 d-flex justify-content-center align-items-center position-relative">
                    ...
                    <form id="pm-upload-form" method="post" enctype="multipart/form-data" class="position-absolute bottom-0 end-0 m-3">
                         ...
                         <label for="pm-upload" class="btn btn-warning shadow"><span class="material-icons">camera_alt</span></label>
                    </form>
                    {% endif %}
                </div>
        ...
        <div class="modal-content-custom" onclick="event.stopPropagation()" style="max-width:800px; padding:20px;">
            <div id="agenda-img-container" style="position:relative; width:100%; margin-bottom:20px;">
                ...
                <a href="https://maps.app.goo.gl/pKfSE3Ewm1RPCDiy9" target="_blank" class="agenda-modal-map-overlay" id="agenda-map-link">
                    klik ini untuk ke lokasi latihan <span class="material-icons">arrow_forward</span> <span class="material-icons fs-5 text-danger">location_on</span>
                </a>
            </div>
        ...
        <div class="modal-content-custom" onclick="event.stopPropagation()">
            ...
            <div style="width:100%; aspect-ratio:16/9; background:#eee; margin-bottom:20px; position:relative; overflow:hidden;">
                ...
                <form action="/upload/history/main" method="post" enctype="multipart/form-data" class="position-absolute bottom-0 end-0 p-2">
                    ...
                    <label for="history-upload" class="btn btn-sm btn-warning"><span class="material-icons">camera_alt</span> Change Image</label>
                </form>
                {% endif %}
            </div>
        ...
        <div class="modal-content-custom" onclick="event.stopPropagation()" style="text-align:left; max-width:900px;">
            ...
            <div style="position:relative;">
                ...
                <form id="news-upload-form" method="post" enctype="multipart/form-data" class="position-absolute bottom-0 end-0 m-3">
                     ...
                     <label for="news-upload" class="btn btn-warning shadow"><span class="material-icons">camera_alt</span></label>
                </form>
                {% endif %}
            </div>
    ...
    <script>
        ...
        function openPartnerModal(index, id, title, details, imagePath, price) {
            ...
            if (isAdmin) {
                ...
                uploadBtn.innerHTML = `<form action="/upload/agenda/${id}" method="post" enctype="multipart/form-data"><label for="p-up-${id}" style="cursor:pointer;width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><span class="material-icons">camera_alt</span></label><input type="file" id="p-up-${id}" name="image" style="display:none;" onchange="this.form.submit()"></form>`;
                ...
                if (index === 0) { 
                    linksHTML = `
                        <a href="instagram://user?username=tahfidzkilatsamarinda" class="agenda-modal-map-overlay" target="_blank" style="text-decoration:none;">
                            klik ini untuk ke sosmednya <span class="material-icons">arrow_forward</span> <span class="material-icons fs-5 text-danger">camera_alt</span>
                        </a>`;
                } else if (index === 1) { 
                    linksHTML = `
                        <div class="agenda-modal-map-overlay" style="cursor:default;">
                            klik ini untuk ke sosmednya <span class="material-icons">arrow_forward</span> 
                            <a href="instagram://user?username=dapurarabiansmd"><span class="material-icons fs-5 text-danger ms-2">camera_alt</span></a>
                            <a href="whatsapp://send?phone=6281241865310"><span class="material-icons fs-5 text-success ms-2">chat</span></a>
                        </div>
                    `;
                } else if (index === 2) { 
                    linksHTML = `
        ...
        function openAgendaModal(id, title, eventDate, price, imagePath) {
            ...
            if (isAdmin) {
                ...
                const uploadBtn = document.createElement('div');
                uploadBtn.className = 'camera-btn';
                uploadBtn.innerHTML = `<form action="/upload/agenda/${id}" method="post" enctype="multipart/form-data"><label for="ag-up-${id}" style="cursor:pointer;width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><span class="material-icons">camera_alt</span></label><input type="file" id="ag-up-${id}" name="image" style="display:none;" onchange="this.form.submit()"></form>`;
...
<body>
    ...
    <div class="container py-5">
        <div class="card shadow border-0 rounded-4">
            <div class="card-header bg-white py-3 border-0">
                <h4 class="fw-bold mb-0 text-success"><span class="material-icons me-2">medical_information</span> Generator Surat Sakit</h4>
            </div>
            <div class="card-body p-4">
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="/surat-sakit/print/{{ p.id }}" method="get" target="_blank" class="d-flex gap-2">
                                        ...
                                        <button type="submit" class="btn btn-sm btn-success"><span class="material-icons me-1">print</span> Print</button>
                                    </form>
                                </td>
                            </tr>
                            {% endfor %}
...
<head>
    ...
    <style>
        body { font-family: 'Plus Jakarta Sans', sans-serif; padding: 40px; max-width: 800px; margin: auto; }
...
<body>
    ...
    <div class="container py-5">
        ...
        <div class="card shadow border-0 rounded-4">
            <div class="card-header bg-white py-3 border-0">
                <h4 class="fw-bold mb-0 text-dark"><span class="material-icons me-2">point_of_sale</span> Transaksi Hari Ini</h4>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr id="row-{{ p.id }}">
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary" onclick="saveFee({{ p.id }})"><span class="material-icons">save</span></button>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
    ...
    <div class="container py-5">
        <div class="card shadow border-0 rounded-4">
            <div class="card-header bg-white py-4 border-0 text-center">
                <h3 class="fw-bold text-primary"><span class="material-icons me-2">storage</span> Database Riwayat Pasien</h3>
                <div class="col-md-6 mx-auto mt-3">
                    <form method="get" class="d-flex gap-2">
                        <input type="text" name="q" class="form-control form-control-lg" placeholder="Cari Nama / No HP..." value="{{ request.args.get('q', '') }}">
                        <button class="btn btn-primary btn-lg"><span class="material-icons">search</span></button>
                    </form>
                </div>
            </div>
            <div class="card-body p-0">
...
<head>
    ...
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <style>
        body{background:#f4f7f6; font-family: 'Plus Jakarta Sans', sans-serif; min-height: 100vh; display: flex; flex-direction: column;}
        .modal-overlay {
            display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
...
<body>
                    ...
                    <div class="card-body p-5 text-center">
                        ...
                        <form method="get" class="mb-5">
                            <div class="input-group input-group-lg">
                                ...
                                <button class="btn btn-primary px-4"><span class="material-icons me-2">search</span> CARI</button>
                            </div>
                        </form>
                        ...
                        <div class="list-group text-start">
                            ...
                            <div class="list-group-item list-group-item-action d-flex justify-content-between align-items-center p-3" style="cursor: pointer;" 
                             onclick='showPlayerCard({{ p | tojson | safe }})'>
                                ...
                                <button class="btn btn-success rounded-pill px-4 fw-bold">PILIH <span class="material-icons ms-2">arrow_forward</span></button>
                            </div>
                            {% endfor %}
                        </div>
                        {% elif request.args.get('q') %}
    ...
    <div id="player-card-modal" class="modal-overlay" onclick="closePlayerCard()">
        <div class="modal-card" onclick="event.stopPropagation()" style="max-width: 500px; max-height: 90vh; overflow-y: auto;">
            <div class="text-end">
                <button class="btn btn-sm btn-danger rounded-circle" onclick="closePlayerCard()"><span class="material-icons">close</span></button>
            </div>
            ...
            <div class="row g-2 text-start mb-4">
                 ...
                 <div class="col-6">
                     <div class="p-2 bg-light rounded border h-100">
                         <small class="text-muted d-block fw-bold" style="font-size:0.7rem;"><span class="material-icons text-info me-1">info</span> STATUS</small>
                 ...
                 <div class="col-6">
                     <div class="p-2 bg-light rounded border h-100">
                         <small class="text-muted d-block fw-bold" style="font-size:0.7rem;"><span class="material-icons text-warning me-1">schedule</span> MASUK/KELUAR</small>
                 ...
                 <div class="col-12">
                     <div class="p-2 bg-white rounded border shadow-sm mt-2">
                        <small class="text-success fw-bold text-uppercase" style="font-size:0.7rem;"><span class="material-icons me-1">medical_information</span> Keluhan</small>
                        <div id="pc-complaint" class="text-dark small fw-bold ms-3">-</div>
                     </div>
                 </div>
                 <div class="col-12">
                     <div class="p-2 bg-white rounded border shadow-sm">
                        <small class="text-primary fw-bold text-uppercase" style="font-size:0.7rem;"><span class="material-icons me-1">medical_services</span> Diagnosa</small>
                        <div id="pc-diagnosis" class="text-dark small fw-bold ms-3">-</div>
                     </div>
                 </div>
                 <div class="col-12">
                     <div class="p-2 bg-white rounded border shadow-sm">
                        <small class="text-danger fw-bold text-uppercase" style="font-size:0.7rem;"><span class="material-icons me-1">medication</span> Resep Obat</small>
                        <div id="pc-prescription" class="text-dark small fw-bold ms-3">-</div>
                     </div>
                 </div>
                 <div class="col-12">
                     <div class="p-2 bg-white rounded border shadow-sm">
                        <small class="text-dark fw-bold text-uppercase" style="font-size:0.7rem;"><span class="material-icons me-1">local_hospital</span> Tindakan</small>
            ...
            <button class="btn btn-success w-100 py-3 fw-bold shadow rounded-pill" id="pc-select-btn">
                <span class="material-icons me-2">check_circle</span> PILIH PASIEN INI
            </button>
        </div>
    </div>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center"><span class="material-icons me-2 text-primary">pie_chart</span> Statistik Penyakit Terbanyak</h2>
...
<head>
    ...
    <style>body{font-family: 'Plus Jakarta Sans', sans-serif; text-align:center; padding-top:50px; background:#f4f7f6;}</style>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
</head>
<body>
...
<body>
    ...
    <div class="container py-5">
        <div class="card shadow border-0 rounded-4" style="max-width: 600px; margin: auto;">
            <div class="card-body p-5">
                <h3 class="fw-bold text-center mb-4 text-primary"><span class="material-icons me-2">event_available</span> Booking Janji Temu</h3>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center"><span class="material-icons me-2 text-success">show_chart</span> Dashboard Keuangan & Tren</h2>
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-danger text-center"><span class="material-icons me-2">warning</span> Tracker Kedaluwarsa Obat</h2>
        <div class="card shadow border-0 rounded-4">
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-dark"><span class="material-icons me-2">receipt</span> Cetak Struk Pembayaran</h2>
        <div class="card shadow border-0 rounded-4">
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="/print-receipt/{{ p.id }}" target="_blank" class="btn btn-sm btn-dark"><span class="material-icons me-1">print</span> Cetak Struk</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
...
<head>
    ...
    <style>
        body { font-family: 'Plus Jakarta Sans', sans-serif; font-size: 12px; width: 58mm; margin: 0; padding: 10px; color: #000; }
...
<body>
    {{ navbar|safe }}
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-center text-success"><span class="material-icons me-2">chat</span> WhatsApp Reminder Pasien</h2>
        <div class="card shadow border-0 rounded-4">
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                                ...
                                <td>
                                    <a href="https://wa.me/{{ p.phone|replace('0', '62', 1) if p.phone.startswith('0') else p.phone }}?text=Halo%20{{ p.name }},%20giliran%20Anda%203%20nomor%20lagi.%20Harap%20bersiap%20di%20ruang%20tunggu.%20Terima%20kasih." 
                                       target="_blank" class="btn btn-success text-white fw-bold">
                                        <span class="material-icons me-1">chat</span> Ingatkan
                                    </a>
                                </td>
                            </tr>
                            {% endfor %}
